...
\caption{附件1 TOPISIS排名（Top 20）}
\label{fig:ranking}
\end{subfigure}
\caption{AHP、熵权与组合赋权的权重对比，以及TOPSIS最终排名。组合赋权有效压制了收入环比增长率的熵权膨胀（0.388$\to$0.184），同时保留了其他指标的合理权重。}
\label{fig:weights_and_ranking}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig_default_rate.png}
\caption{风险等级与违约率的单调性验证。柱状图显示各等级企业数量与违约数量，折线显示违约率从A级10.0\%单调递增至D级56.0\%。这是模型有效性的核心证据。}
\label{fig:default_rate}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{fig_crosstab.png}
\caption{模型评级与银行原始评级交叉热力图。对角线上的深色格表示两种评级一致的企业，大部分差异在相邻等级内。}
\label{fig:crosstab}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig_case_radar.png}
\caption{三家典型企业（E38最优、E64预警、E115违约）的六维风险雷达图。E38在所有维度接近满分；E64的利润率、稳定性和增长趋势维度显著弱化；E115全面崩溃。}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{fig_rate_churn.png}
\caption{三个信誉等级的利率--流失率拟合曲线。三条曲线均拟合优良（$R^2>0.99$）。在4\%利率时流失率接近0，在15\%时约90\%，为差异化定价提供了精确的量化工具。}
\label{fig:rate_churn}
\end{figure}
...
\caption{疫情对各行业贷款额度的影响。严重负面行业额度骤降44\%，正面行业额度上升22\%，反映了将有限信贷资源向抗风险能力强的行业倾斜的危机管理逻辑。}
\label{fig:covid}
\end{figure}
...
\caption{100次随机权重扰动（$\pm$10\%）的Spearman相关系数分布。均值0.965远高于0.85稳健性阈值，最小值为0.949，证明模型排名对权重设定不敏感。}
